...
public class TimingTrackingApplicationContextInitializer implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    ...
    /**
     * Gets the recorded load time for a context, if available.
     */
    public static Long getContextLoadTime(ConfigurableApplicationContext context) {
        String contextId = context.getClass().getSimpleName() + "@" + System.identityHashCode(context);
        return contextLoadTimes.get(contextId);
    }
    
    /**
     * Gets comprehensive profile data for a context.
     */
    ...
    /**
     * Gets all recorded profile data.
     */
    public static Map<String, ContextProfileData> getAllProfileData() {
        return new ConcurrentHashMap<>(contextProfileData);
    }
    
    /**
     * Clears all recorded profiling data (for testing).
     */
    public static void clearTimingData() {
        contextStartTimes.clear();
        contextLoadTimes.clear();
        contextProfileData.clear();
        contextStartMemory.clear();
    }
}